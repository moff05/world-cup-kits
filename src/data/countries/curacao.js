export const curacao = {
  "id": "curacao",
  "name": "Curaçao",
  "flagCode": "cw",
  "confederation": "CONCACAF",
  "worldCups": [
    2026
  ],
  "bestResult": "Group Stage (2026)",
  "stats": {
    "matches": 3,
    "wins": 0,
    "draws": 1,
    "losses": 2,
    "goalsFor": 1,
    "goalsAgainst": 9
  },
  "facts": [
    {
      "id": "born-from-a-dissolution",
      "label": "BORN FROM A DISSOLUTION",
      "story": "Curaçao's national team didn't exist until 2011. For decades the island competed as part of the Netherlands Antilles, a federation of six Dutch Caribbean territories that dissolved on 10 October 2010. Curaçao and Sint Maarten became separate countries, and the new Curaçao Football Federation fielded its first squad on 20 August 2011 against the Dominican Republic — a 1–0 loss in San Cristóbal that marked the birth of an entirely new footballing nation."
    },
    {
      "id": "gold-cup-breakthrough",
      "label": "SMALLEST NATION, BIG RUN",
      "story": "Curaçao's rise came fast for such a young federation. After winning the 2017 Caribbean Cup by beating Jamaica, the team earned its first-ever CONCACAF Gold Cup berth, then lost all three group games in its 2017 debut. Two years later, in 2019, Curaçao reached the Gold Cup quarterfinals — a remarkable leap for an island of roughly 150,000 people fielding one of the region's newest federations."
    }
  ],
  "kits": {
    "2026": {
      "result": "Group Stage",
      "headline": "Smallest Nation, Historic Debut",
      "homeColors": "lucid blue shirt with wave-pattern sleeves, blue shorts, blue socks with teal and yellow trim",
      "awayColors": "almost yellow shirt with turquoise, orange and pink shoulder stripes, pale yellow shorts, pale yellow socks",
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Germany",
          "score": "1–7",
          "result": "L",
          "scorers": "Comenencia 21'"
        },
        {
          "round": "Group Stage",
          "opponent": "Ecuador",
          "score": "0–0",
          "result": "D",
          "scorers": null
        },
        {
          "round": "Group Stage",
          "opponent": "Ivory Coast",
          "score": "0–2",
          "result": "L",
          "scorers": null
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": null,
          "annotations": [
            {
              "id": "rooms-record-night",
              "label": "THE KEY MOMENT",
              "story": "Down 0–0 against a rampant Ecuador side that fired 26 shots and held 75 percent of possession, goalkeeper Eloy Room produced a tournament-record-equaling 15 saves in Kansas City. The shutout delivered Curaçao's first-ever World Cup point — a stunning rearguard performance from the smallest nation ever to reach the tournament."
            },
            {
              "id": "island-identity-kit",
              "label": "THE KIT",
              "story": "Adidas built Curaçao's debut kit around 'Island Identity.' The lucid blue home shirt carries wavy sleeve graphics echoing the Caribbean surf, trimmed in impact yellow. The pale yellow away shirt flips the palette, with turquoise, orange and pink shoulder stripes lifted from the island's brightly painted colonial architecture."
            },
            {
              "id": "smallest-nation-ever",
              "label": "THE CONTEXT",
              "story": "At roughly 156,000 people, Curaçao became the smallest nation by population and area ever to qualify for a World Cup, clinching its spot on November 18, 2025. The run was steered by 78-year-old Dick Advocaat, who became the oldest manager to take charge of a World Cup match when Curaçao opened against Germany."
            }
          ]
        }
      }
    }
  }
};
