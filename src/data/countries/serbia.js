export const serbia = {
  "id": "serbia",
  "name": "Serbia",
  "flagCode": "rs",
  "confederation": "UEFA",
  "worldCups": [
    1930,
    1950,
    1954,
    1958,
    1962,
    1974,
    1982,
    1990,
    1998,
    2006,
    2010,
    2018,
    2022
  ],
  "bestResult": "Fourth Place (1930, 1962)",
  "stats": {
    "matches": 49,
    "wins": 18,
    "draws": 9,
    "losses": 22,
    "goalsFor": 71,
    "goalsAgainst": 71
  },
  "facts": [
    {
      "id": "plavi-to-orlovi",
      "label": "PLAVI TO ORLOVI",
      "story": "For 86 years, from the 1920s until 2006, the national team wore blue and carried the nickname \"Plavi\" — the Blues. When Montenegro split from Serbia that year, the federation switched to a red shirt with blue shorts and white socks, honoring the Serbian tricolor, and the crest gained a white double-headed eagle. Fans renamed the side \"Orlovi,\" the Eagles. In 2025, nineteen years later, Serbia brought blue back into the kit, reviving the old nickname alongside the new one."
    },
    {
      "id": "los-ichachos",
      "label": "LOS ICHACHOS",
      "story": "At the inaugural 1930 World Cup in Montevideo, Yugoslavia fielded the youngest squad in the tournament, averaging barely 22 years old, yet opened with a stunning 2–1 win over Brazil at Parque Central. The scrappy, fast-attacking teenagers charmed the host country's crowds so much that Uruguayan fans gave them a nickname of their own: \"Los Ichachos.\" The affection didn't save them from a 6–1 semi-final beating at the hands of the tournament hosts, but the nickname outlived the result."
    }
  ],
  "kits": {
    "1930": {
      "result": "4th Place",
      "headline": "Teenagers Topple Brazil",
      "homeColors": "dark blue shirt, white shorts, red socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Brazil",
          "score": "2–1",
          "result": "W",
          "scorers": "Tirnanić 21', Bek 30'"
        },
        {
          "round": "Group Stage",
          "opponent": "Bolivia",
          "score": "4–0",
          "result": "W",
          "scorers": "Bek 60', Marjanović 65', Bek 67', Vujadinović 85'"
        },
        {
          "round": "Semi-final",
          "opponent": "Uruguay",
          "score": "1–6",
          "result": "L",
          "scorers": "Vujadinović 4'"
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "the-vanishing-goal",
              "label": "THE VANISHING GOAL",
              "story": "Down 3–1 to Uruguay in the semi-final, Yugoslavia had a goal waved off just before halftime on a tight offside call, and one account has a policeman kicking a stray ball back into play to help set up a Uruguayan strike. Furious, Yugoslavia refused to contest the third-place match entirely."
            },
            {
              "id": "no-logo-no-sponsor",
              "label": "NO LOGO, NO SPONSOR",
              "story": "The dark blue jersey worn in Montevideo carried no manufacturer's mark of any kind — Yugoslavia's kits stayed entirely in-house, stitched without a single brand stripe or crest sponsor, for 38 more years. Not until 1968 would a commercial kit maker's name ever touch the shirt."
            },
            {
              "id": "three-weeks-by-sea",
              "label": "THREE WEEKS BY SEA",
              "story": "With the passenger liner Conte Verde already booked solid by France, Belgium, and Romania, Yugoslavia's squad took a three-day train to Marseille and boarded the pleasure cruiser Florida alone. Egypt was meant to join them; their own ship was delayed by weather and the Florida sailed without them."
            }
          ]
        }
      }
    },
    "1950": {
      "result": "Group Stage",
      "headline": "Denied By Rio Heartbreak",
      "homeColors": "dark blue shirt, white shorts, red socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Switzerland",
          "score": "3–0",
          "result": "W",
          "scorers": "Mitić 59', Tomašević 70', Ognjanov 84'"
        },
        {
          "round": "Group Stage",
          "opponent": "Mexico",
          "score": "4–1",
          "result": "W",
          "scorers": "Bobek 20', Čajkovski 23', Čajkovski 51', Tomašević 81'"
        },
        {
          "round": "Group Stage",
          "opponent": "Brazil",
          "score": "0–2",
          "result": "L",
          "scorers": null
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "ten-men-in-rio",
              "label": "TEN MEN IN RIO",
              "story": "Captain Rajko Mitić gashed his head on a low iron beam leaving the tunnel before kickoff against Brazil and had to be stitched up pitchside. The referee refused to delay kickoff, so Yugoslavia played the opening minutes with ten men and never fully recovered, losing 2–0."
            },
            {
              "id": "the-colors-that-survived",
              "label": "THE COLORS THAT SURVIVED",
              "story": "Twenty years after the team's first match, the same dark blue shirt, white shorts, and red socks took the field in Brazil — unchanged through a world war, a Nazi occupation, and the collapse of the monarchy into a socialist republic under Tito."
            },
            {
              "id": "breaking-from-moscow",
              "label": "BREAKING FROM MOSCOW",
              "story": "Two years after Stalin's Cominform expelled Yugoslavia over Tito's refusal to bow to Soviet control, triggering a crushing economic embargo, Yugoslavia was the only Eastern Bloc nation to show up in Brazil — the USSR, Hungary, and Czechoslovakia all stayed home."
            }
          ]
        }
      }
    },
    "1954": {
      "result": "Quarter-final",
      "headline": "Halted By The Miracle Men",
      "homeColors": "dark blue shirt, white shorts, red socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "France",
          "score": "1–0",
          "result": "W",
          "scorers": "Milutinović 15'"
        },
        {
          "round": "Group Stage",
          "opponent": "Brazil",
          "score": "1–1",
          "result": "D",
          "scorers": "Zebec 48'"
        },
        {
          "round": "Quarter-final",
          "opponent": "West Germany",
          "score": "0–2",
          "result": "L",
          "scorers": null
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "milutinovics-opener",
              "label": "MILUTINOVIĆ'S OPENER",
              "story": "Miloš Milutinović's 15th-minute strike was enough to sink France in Lausanne, and Branko Zebec's second-half leveler against tournament favorites Brazil held Yugoslavia to a 1–1 draw — a rare point taken off the eventual runners-up before West Germany's defense shut them out in the quarter-final."
            },
            {
              "id": "two-decades-before-a-logo",
              "label": "TWO DECADES BEFORE A LOGO",
              "story": "The navy shirt worn in Switzerland was still made in-house, with two more decades to pass before Yugoslavia wore its first branded manufacturer's kit in 1974. No stripes, no swoosh — just plain wool and a crest, unchanged since the team's founding."
            },
            {
              "id": "nine-years-after-liberation",
              "label": "NINE YEARS AFTER LIBERATION",
              "story": "Less than a decade removed from Nazi occupation and a brutal partisan resistance, Tito's rebuilding Yugoslavia sent a team to Switzerland as a statement of national recovery — a socialist state charting its own path, distinct from Moscow, on football's biggest stage."
            }
          ]
        }
      }
    },
    "1958": {
      "result": "Quarter-final",
      "headline": "Petaković Can't Break Germany",
      "homeColors": "dark blue shirt, white shorts, red socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Scotland",
          "score": "1–1",
          "result": "D",
          "scorers": "Petaković 6'"
        },
        {
          "round": "Group Stage",
          "opponent": "France",
          "score": "3–2",
          "result": "W",
          "scorers": "Petaković 16', Veselinović 63', Veselinović 88'"
        },
        {
          "round": "Group Stage",
          "opponent": "Paraguay",
          "score": "3–3",
          "result": "D",
          "scorers": "Ognjanović 18', Veselinović 21', Rajkov 73'"
        },
        {
          "round": "Quarter-final",
          "opponent": "West Germany",
          "score": "0–1",
          "result": "L",
          "scorers": null
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "petakovics-streak",
              "label": "PETAKOVIĆ'S SCORING STREAK",
              "story": "Aleksandar Petaković found the net in three consecutive matches, dragging Yugoslavia through a bruising group against Scotland, France, and Paraguay that finished with two draws and a win. West Germany's back line proved the one defense he couldn't crack, shutting him out 1–0 in the quarter-final."
            },
            {
              "id": "the-plain-white-change",
              "label": "THE PLAIN WHITE CHANGE",
              "story": "Yugoslavia's away kit in Sweden was a plain white long-sleeved jersey with a V-neck collar, still made in-house, with a multicolored crest stitched prominently on the left chest — the only splash of color on an otherwise stripped-down, unbranded shirt."
            },
            {
              "id": "a-bridge-across-blocs",
              "label": "A BRIDGE ACROSS BLOCS",
              "story": "Tito's independent socialist line kept Yugoslavia diplomatically useful to both Cold War camps, and its footballers reflected that: a communist-governed side sharing a group with Scotland and France, comfortably embedded in a Western European tournament calendar that shut out most of the Eastern Bloc."
            }
          ]
        }
      }
    },
    "1962": {
      "result": "4th Place",
      "headline": "Jerković's Golden Boot Run",
      "homeColors": "dark blue shirt, white shorts, red socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Soviet Union",
          "score": "0–2",
          "result": "L",
          "scorers": null
        },
        {
          "round": "Group Stage",
          "opponent": "Uruguay",
          "score": "3–1",
          "result": "W",
          "scorers": "Skoblar 25', Galić 29', Jerković 49'"
        },
        {
          "round": "Group Stage",
          "opponent": "Colombia",
          "score": "5–0",
          "result": "W",
          "scorers": "Galić 20', Jerković 25', Galić 61', Melić 82', Jerković 87'"
        },
        {
          "round": "Quarter-final",
          "opponent": "West Germany",
          "score": "1–0",
          "result": "W",
          "scorers": "Radaković 85'"
        },
        {
          "round": "Semi-final",
          "opponent": "Czechoslovakia",
          "score": "1–3",
          "result": "L",
          "scorers": "Jerković 69'"
        },
        {
          "round": "third-place match",
          "opponent": "Chile",
          "score": "0–1",
          "result": "L",
          "scorers": null
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "jerkovics-golden-run",
              "label": "JERKOVIĆ'S GOLDEN RUN",
              "story": "Dražan Jerković scored four times to fire Yugoslavia past Uruguay and Colombia, and Milan Radaković's 85th-minute strike stunned West Germany in the quarter-final. Czechoslovakia ended the run in the semi, and a spent Yugoslavia lost the third-place match to hosts Chile, settling for fourth again."
            },
            {
              "id": "the-tricolor-crest",
              "label": "THE TRICOLOR CREST",
              "story": "Yugoslavia's away kit in Chile was a plain white jersey with a V-neck collar, still stitched in-house with no manufacturer's name, but its left-chest crest burst with red, yellow, and blue — the only vivid detail on an otherwise minimalist shirt."
            },
            {
              "id": "a-year-after-belgrade",
              "label": "A YEAR AFTER BELGRADE",
              "story": "Just months after Belgrade hosted the founding summit of the Non-Aligned Movement in 1961, Tito's Yugoslavia arrived in South America positioning itself as a bridge between capitalist and communist blocs — a diplomatic balancing act mirrored by a football team that beat both West Germany and Uruguay."
            }
          ]
        }
      }
    },
    "1974": {
      "result": "2nd Round",
      "headline": "Nine Unanswered Goals",
      "homeColors": "dark blue shirt, white shorts, red socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Brazil",
          "score": "0–0",
          "result": "D",
          "scorers": null
        },
        {
          "round": "Group Stage",
          "opponent": "Zaire",
          "score": "9–0",
          "result": "W",
          "scorers": "Bajević 8', Džajić 14', Šurjak 18', Katalinski 22', Bajević 30', Bogićević 35', Oblak 61', Petković 65', Bajević 81'"
        },
        {
          "round": "Group Stage",
          "opponent": "Scotland",
          "score": "1–1",
          "result": "D",
          "scorers": "Karasi 81'"
        },
        {
          "round": "2nd Round",
          "opponent": "West Germany",
          "score": "0–2",
          "result": "L",
          "scorers": null
        },
        {
          "round": "2nd Round",
          "opponent": "Poland",
          "score": "1–2",
          "result": "L",
          "scorers": "Karasi 43'"
        },
        {
          "round": "2nd Round",
          "opponent": "Sweden",
          "score": "1–2",
          "result": "L",
          "scorers": "Šurjak 27'"
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "nine-goals-one-record",
              "label": "NINE GOALS, ONE RECORD",
              "story": "Dušan Bajević's hat-trick anchored a 9–0 demolition of Zaire that stands as the biggest win in the program's World Cup history, a rout with nine different sequences finding the net after a scoreless draw with Brazil. A stacked second group with West Germany, Poland, and Sweden ended the run just short of the semis."
            },
            {
              "id": "the-first-three-stripes",
              "label": "THE FIRST THREE STRIPES",
              "story": "After 44 years of unbranded, in-house jerseys, 1974 marked Yugoslavia's first-ever manufacturer kit: an Adidas shirt with the German brand's three stripes trimming the sleeves, joining the era's dominant football supplier for the first time in the team's history."
            },
            {
              "id": "brotherhood-on-the-pitch",
              "label": "BROTHERHOOD ON THE PITCH",
              "story": "The 1974 squad drew players from across Yugoslavia's constituent socialist republics, a multi-ethnic roster fielded at the high-water mark of Tito's 'Brotherhood and Unity' era — decades before the country's ethnic divisions would tear both the nation and its football apart."
            }
          ]
        }
      }
    },
    "1982": {
      "result": "Group Stage",
      "headline": "A Quiet Group Stage Exit",
      "homeColors": "dark blue shirt, white shorts, red socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Northern Ireland",
          "score": "0–0",
          "result": "D",
          "scorers": null
        },
        {
          "round": "Group Stage",
          "opponent": "Spain",
          "score": "1–2",
          "result": "L",
          "scorers": "Gudelj 10'"
        },
        {
          "round": "Group Stage",
          "opponent": "Honduras",
          "score": "1–0",
          "result": "W",
          "scorers": "Petrović 88'"
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "gudeljs-lone-strike",
              "label": "GUDELJ'S LONE STRIKE",
              "story": "Petar Gudelj's goal against host nation Spain wasn't enough to prevent a 2–1 loss, and a scoreless opener against Northern Ireland left little margin. Petrović's late winner beat Honduras to close the group, but the numbers didn't add up — Yugoslavia went out on goal difference."
            },
            {
              "id": "pinstripes-and-three-stripes",
              "label": "PINSTRIPES AND THREE STRIPES",
              "story": "Yugoslavia's Adidas away kit in Spain was a white shirt threaded with thin blue pinstripes, a classic collar, and red-and-blue Adidas stripes across the shoulders — a subtler design than the plain navy home shirt the team had worn for half a century."
            },
            {
              "id": "playing-in-mourning",
              "label": "PLAYING IN MOURNING",
              "story": "This was the first World Cup Yugoslavia entered after Josip Broz Tito's death in May 1980, the leader whose decades-long grip had held the country's six republics together. The quiet group-stage exit came as the nation he built began, almost imperceptibly, to drift toward the fractures of the following decade."
            }
          ]
        }
      }
    },
    "1990": {
      "result": "Quarter-final",
      "headline": "Stojković Stuns Spain",
      "homeColors": "dark blue shirt, white shorts, red socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "West Germany",
          "score": "1–4",
          "result": "L",
          "scorers": "Jozić 55'"
        },
        {
          "round": "Group Stage",
          "opponent": "Colombia",
          "score": "1–0",
          "result": "W",
          "scorers": "Jozić 75'"
        },
        {
          "round": "Group Stage",
          "opponent": "United Arab Emirates",
          "score": "4–1",
          "result": "W",
          "scorers": "Sušić 5', Pančev 9', Pančev 46', Prosinečki 90'+3'"
        },
        {
          "round": "Round of 16",
          "opponent": "Spain",
          "score": "2–1",
          "result": "W",
          "scorers": "Stojković 78', Stojković 93'"
        },
        {
          "round": "Quarter-final",
          "opponent": "Argentina",
          "score": "0–0 (2–3 pens)",
          "result": "D",
          "scorers": null
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "stojkovics-double",
              "label": "STOJKOVIĆ'S EXTRA-TIME DOUBLE",
              "story": "Dragan Stojković scored in the 78th and 93rd minutes to sink Spain 2–1 in extra time, sending Yugoslavia into a quarter-final against Diego Maradona's Argentina. Neither side broke through in 120 goalless minutes in Florence, and Argentina survived the shootout 3–2 to end the run."
            },
            {
              "id": "the-lightning-bolt-shirt",
              "label": "THE LIGHTNING BOLT SHIRT",
              "story": "Yugoslavia's Adidas away kit that summer was a white shirt slashed with red-and-blue lightning-bolt graphics down the sides and a blue V-neck collar — a bold departure from the plain navy home strip the team had worn largely unchanged since 1930."
            },
            {
              "id": "the-boban-absence",
              "label": "THE BOBAN ABSENCE",
              "story": "Weeks before the tournament, Dinamo Zagreb captain Zvonimir Boban kicked a policeman during the Maksimir riot between Dinamo and Red Star fans, an incident tied to Croatia's independence movement. His six-month ban kept him off the 1990 squad — a preview of the breakup to come."
            }
          ]
        }
      }
    },
    "1998": {
      "result": "Round of 16",
      "headline": "Komljenović's Breakthrough Run",
      "homeColors": "dark blue shirt, white shorts, red socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Iran",
          "score": "1–0",
          "result": "W",
          "scorers": "Mihajlović 73'"
        },
        {
          "round": "Group Stage",
          "opponent": "Germany",
          "score": "2–2",
          "result": "D",
          "scorers": "Mijatović 13', Stojković 52'"
        },
        {
          "round": "Group Stage",
          "opponent": "United States",
          "score": "1–0",
          "result": "W",
          "scorers": "Komljenović 4'"
        },
        {
          "round": "Round of 16",
          "opponent": "Netherlands",
          "score": "1–2",
          "result": "L",
          "scorers": "Komljenović 48'"
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "komljenovics-double-duty",
              "label": "KOMLJENOVIĆ'S BREAKTHROUGH",
              "story": "Slobodan Komljenović's solitary strike beat the United States, and he found the net again against the Netherlands in the Round of 16 — but Edgar Davids's 92nd-minute winner sent the Dutch through 2–1. Predrag Mijatović and Dragan Stojković had earlier salvaged a 2–2 draw with Germany."
            },
            {
              "id": "one-central-stripe",
              "label": "ONE CENTRAL STRIPE",
              "story": "The Adidas away shirt worn in France was mostly white with a single dark navy stripe running down the center, a V-neck trimmed in blue, white, and red, and matching three stripes on the shoulders — a design pared back from the loud graphics of 1990."
            },
            {
              "id": "the-lost-generations-reunion",
              "label": "THE LOST GENERATION'S REUNION",
              "story": "UN sanctions had expelled Yugoslavia from all international football in 1992 over the Bosnian war, wiping out an entire golden generation that never played Euro 92 or USA 94. This was that generation's only real World Cup — reassembled under a new flag, a rump federation of just Serbia and Montenegro."
            }
          ]
        }
      }
    },
    "2006": {
      "result": "Group Stage",
      "headline": "Argentina's Six-Goal Humiliation",
      "homeColors": "dark blue shirt, white shorts, red socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Netherlands",
          "score": "0–1",
          "result": "L",
          "scorers": null
        },
        {
          "round": "Group Stage",
          "opponent": "Argentina",
          "score": "0–6",
          "result": "L",
          "scorers": null
        },
        {
          "round": "Group Stage",
          "opponent": "Ivory Coast",
          "score": "2–3",
          "result": "L",
          "scorers": "Žigić 10', Ilić 20'"
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "the-worst-defeat-on-record",
              "label": "THE WORST DEFEAT ON RECORD",
              "story": "A 1–0 opening loss to the Netherlands was merely a warm-up for what followed: a 6–0 demolition by Argentina that remains the heaviest defeat in the program's World Cup history. Žigić and Ilić's consolation goals in a 2–3 finale loss to Ivory Coast closed out a winless tournament."
            },
            {
              "id": "lottos-farewell-kit",
              "label": "LOTTO'S FAREWELL KIT",
              "story": "Made by Italian manufacturer Lotto, this shirt would be the last one ever to carry the name 'Serbia and Montenegro' — a jersey representing a federation that, by the time the tournament kicked off, had already voted itself out of existence."
            },
            {
              "id": "playing-a-country-that-no-longer-existed",
              "label": "A COUNTRY THAT NO LONGER EXISTED",
              "story": "Montenegro's independence referendum passed on May 21, 2006, just over two weeks before the tournament began. Serbia and Montenegro's players took the field in Germany representing a state that had legally dissolved before their first match — the last gasp of a union already over."
            }
          ]
        }
      }
    },
    "2010": {
      "result": "Group Stage",
      "headline": "Jovanović Topples Germany",
      "homeColors": "red shirt, blue shorts, white socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Ghana",
          "score": "0–1",
          "result": "L",
          "scorers": null
        },
        {
          "round": "Group Stage",
          "opponent": "Germany",
          "score": "1–0",
          "result": "W",
          "scorers": "Jovanović 38'"
        },
        {
          "round": "Group Stage",
          "opponent": "Australia",
          "score": "1–2",
          "result": "L",
          "scorers": "Pantelić 84'"
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "jovanovics-shock",
              "label": "JOVANOVIĆ'S SHOCK WINNER",
              "story": "Milan Jovanović's 38th-minute strike downed defending runners-up Germany 1–0, one of the biggest upsets of the tournament. It wasn't enough — an opening loss to Ghana and a 2–1 defeat to Australia, with only a Pantelić consolation goal, sent Serbia out on goal difference anyway."
            },
            {
              "id": "the-first-red-shirt",
              "label": "THE FIRST TRUE RED SHIRT",
              "story": "This was Serbia's first World Cup as a standalone nation, and the Nike-made kit reflected it: a vibrant red shirt with a bold white cross design and subtle blue trim, replacing the century-old dark blue jersey the team had worn as Yugoslavia and Serbia and Montenegro."
            },
            {
              "id": "a-flag-still-being-drawn",
              "label": "A FLAG STILL BEING DRAWN",
              "story": "Serbia arrived in South Africa as its own country for the first time since Montenegro's split four years earlier, while Kosovo's 2008 declaration of independence remained unrecognized by Belgrade — the borders of the former Yugoslavia still being contested even as this new red shirt made its debut."
            }
          ]
        }
      }
    },
    "2018": {
      "result": "Group Stage",
      "headline": "Kolarov's Free Kick Opener",
      "homeColors": "red shirt, blue shorts, white socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Costa Rica",
          "score": "1–0",
          "result": "W",
          "scorers": "Kolarov 56'"
        },
        {
          "round": "Group Stage",
          "opponent": "Switzerland",
          "score": "1–2",
          "result": "L",
          "scorers": "Mitrović 5'"
        },
        {
          "round": "Group Stage",
          "opponent": "Brazil",
          "score": "0–2",
          "result": "L",
          "scorers": null
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "kolarovs-opener",
              "label": "KOLAROV'S FREE-KICK OPENER",
              "story": "Aleksandar Kolarov's dead-ball strike in the 56th minute beat Costa Rica to open the tournament well, but it unraveled fast: Xherdan Shaqiri's late winner flipped a 1–0 lead into a 2–1 loss to Switzerland, and Brazil closed the book with a 2–0 win in Moscow."
            },
            {
              "id": "the-twenty-euro-jersey",
              "label": "THE TWENTY-EURO TEMPLATE",
              "story": "Puma's debut Serbia kit was built directly on the brand's cheap 'Liga' teamwear template — the same generic block sold to amateur clubs for roughly €20 — leaving a shirt so plain and unbranded-looking that it ranks among the lowest-rated World Cup jerseys ever cataloged."
            },
            {
              "id": "shaqiris-double-eagle",
              "label": "SHAQIRI'S DOUBLE EAGLE",
              "story": "After scoring the winner, Xherdan Shaqiri — born in Kosovo to ethnic Albanian parents — crossed his hands to mimic the double-headed eagle of the Albanian flag, a gesture teammate Granit Xhaka repeated moments later. The celebration reignited raw tensions from the 1998–99 Kosovo war on football's biggest stage."
            }
          ]
        }
      }
    },
    "2022": {
      "result": "Group Stage",
      "headline": "Mitrović's Late Cameroon Rescue",
      "homeColors": "red shirt, blue shorts, white socks",
      "awayColors": null,
      "matches": [
        {
          "round": "Group Stage",
          "opponent": "Brazil",
          "score": "0–2",
          "result": "L",
          "scorers": null
        },
        {
          "round": "Group Stage",
          "opponent": "Cameroon",
          "score": "3–3",
          "result": "D",
          "scorers": "Pavlović 45+1', Milinković-Savić 45+3', Mitrović 53'"
        },
        {
          "round": "Group Stage",
          "opponent": "Switzerland",
          "score": "2–3",
          "result": "L",
          "scorers": "Mitrović 26', Vlahović 35'"
        }
      ],
      "kits": {
        "home": {
          "image": null,
          "imageCredit": {
            "player": null,
            "game": null
          },
          "annotations": [
            {
              "id": "the-cameroon-thriller",
              "label": "THE GAME OF THE TOURNAMENT",
              "story": "Two stoppage-time goals from Strahinja Pavlović and Sergej Milinković-Savić, plus a Mitrović finish early in the second half, pulled Serbia back from 2–1 down to lead Cameroon 3–1 — only for two late Cameroon goals to salvage a wild 3–3 draw, one of the tournament's highest-scoring games."
            },
            {
              "id": "the-lowest-rated-shirt",
              "label": "THE LOWEST-RATED SHIRT",
              "story": "Puma's Serbia away kit for Qatar, a white-and-gold design dominated by a boxy front panel meant to frame the squad numbers, went on to become one of the worst-reviewed jerseys in the entire Football Kit Archive database, drawing barely 2 out of 5 across more than 1,300 votes."
            },
            {
              "id": "the-locker-room-banner",
              "label": "THE LOCKER ROOM BANNER",
              "story": "Before facing Brazil, Serbia's federation hung a map of Serbia including Kosovo, captioned 'No Surrender,' inside the team's locker room. FIFA charged the federation for the political display after Kosovo's government called it hateful — reviving a dispute rooted in the 1998–99 war that had never fully cooled."
            }
          ]
        }
      }
    }
  }
};
